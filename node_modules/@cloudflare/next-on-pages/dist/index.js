...
// src/utils/paths.ts
var normalizePath = (path) => path.startsWith("\\\\?\\") ? path : path.replace(/\\/g, "/");

// src/utils/transform/manifest.ts
var matchFunctionEntry = (entryName, fileName) => {
  if (entryName.startsWith("app/")) {
    const type = entryName.endsWith("/route") ? "/route" : "/page";
    return `app${fileName !== "index" ? `/${fileName}` : ""}${type}` === entryName;
  }
    ...
    await Promise.all(
      files.map(async (file) => {
        ...
        if (isDirectory && filepath.endsWith(".func")) {
          ...
          let functionConfig;
          try {
            const contents2 = await (0, import_promises.readFile)(functionConfigFile, "utf8");
